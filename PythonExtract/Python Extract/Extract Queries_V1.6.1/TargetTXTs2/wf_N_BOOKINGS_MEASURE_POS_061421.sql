...
AND WBM.SK_SALES_MOTION_ATTRIB_KEY < 0 ; /* Added to Qualify Bundle and Offset Records */
...
/* ADDED AS PART OF POS DSV CHANGES */

/*Update added as part of May 16th release*/
...
AND MT.DV_ENTERPRISE_INV_SKU_ID = WBM.DV_PRODUCT_KEY
AND MT.DV_TRX_TYPE = WBM.BKGS_MEASURE_TRANS_TYPE_CODE  
--AND MT.SK_OFFER_ATTRIBUTION_ID_INT = WBM.SK_OFFER_ATTRIBUTION_ID_INT
AND WBM.BKGS_MEASURE_TRANS_TYPE_CODE ='POS' 
AND WBM.DV_ATTRIBUTION_CD IN ('STANDALONE');
...
$$WORKDB.W_BOOKINGS_MEASURE WBM,
 ( SELECT 
	...
	DV_TRX_TYPE ,
	NSOL.RTNR_ELGIBILITY_FLG RTNR_ELGIBILITY_FLG ,
MT.SK_OFFER_ATTRIBUTION_ID_INT
	FROM $$SLSORDVWDB.MT_BKG_SLS_MOTION_ATTRIBUTION  MT
INNER JOIN $$SLSORDVWDB.N_SALES_ORDER_LINE NSOL
  ON CASE WHEN MT.SALES_ORDER_LINE_KEY < 0 THEN -7777 ELSE MT.SALES_ORDER_LINE_KEY END = NSOL.SALES_ORDER_LINE_KEY	
	WHERE DV_TRX_TYPE = 'POS'
...
AND MT.SK_OFFER_ATTRIBUTION_ID_INT = WBM.SK_OFFER_ATTRIBUTION_ID_INT
AND WBM.BKGS_MEASURE_TRANS_TYPE_CODE ='POS' 
AND WBM.DV_ATTRIBUTION_CD IN ('ATTRIBUTED');
...
UPDATE WBM 
 ...
 (SELECT BK_POS_TRANSACTION_ID_INT 
   ...
   WHERE SALES_ORDER_LINE_KEY IN (SELECT SALES_ORDER_LINE_KEY  FROM $$SLSORDVWDB.N_SALES_ORDER_LINE NSOL
                                  WHERE SALES_ORDER_LINE_KEY > 0 AND RTNR_ELGIBILITY_FLG = 'N' AND NSOL.SS_CODE IN('CG','OPL')
...
AND NBM.BKGS_MEASURE_TRANS_TYPE_CODE ='POS'
AND NBM.BOOKINGS_PROCESS_DATE = FDY.CALENDAR_DATE
--AND MT.SK_OFFER_ATTRIBUTION_ID_INT = NBM.SK_OFFER_ATTRIBUTION_ID_INT
AND FDY.FISCAL_YEAR_NUMBER_INT >= 2016
AND NBM.DV_ATTRIBUTION_CD IN ('STANDALONE') ; */
...
------------------------------------------------------------------------------------------------
----------- For BUNDLE Lines
DELETE FROM $$STGDB.WI_NBM_INC WHERE BKGS_MEASURE_TRANS_TYPE_CODE='POS';
...
NBM.BOOKINGS_MEASURE_KEY,
NBM.BOOKINGS_PROCESS_DATE,
-9999 as DV_SALES_ORDER_LINE_KEY,
NBM.BK_POS_TRANSACTION_ID_INT,
...
WHERE GOODS_OR_SERVICE_TYPE = 'GOODS'  AND MONETIZATION_TYPE_CD = 'TERM' AND PRODUCT_CATEGORY_CD = 'SOFTWARE') P  
ON ( P.ITEM_KEY = NBM.PRODUCT_KEY  AND NBM.BOOKINGS_PROCESS_DATE BETWEEN P.START_TV_DATE AND P.END_TV_DATE)
LEFT JOIN (SELECT 
...
GROUP BY 1,2,3) PC 
ON (PC.PRODUCT_KEY = NBM.PRODUCT_KEY AND PC.DV_FISCAL_YEAR_QUARTER_NUM_INT = FMP.FISCAL_YEAR_QUARTER_NUMBER_INT)
WHERE 1=1
...
AND (( NBM.SERVICE_FLG = 'N' AND (PC.MONETIZATION_TYPE_CD = 'TERM'  OR (P.MONETIZATION_TYPE_CD = 'TERM' AND P.PRODUCT_CATEGORY_CD = 'SOFTWARE')))
    ...
    AND NOT EXISTS  ( 
        SELECT 1 FROM $$STGDB.WI_NBM_INC INC WHERE   
                   ( INC.BK_POS_TRANSACTION_ID_INT = NBM.BK_POS_TRANSACTION_ID_INT AND INC.DV_ATTRIBUTION_CD NOT IN ('STANDALONE','ATTRIBUTED')
...
COLLECT STATS ON $$STGDB.WI_NBM_INC COLUMN (BK_POS_TRANSACTION_ID_INT ) ;
...
--------------- Forward linesand '=' enrichment
DELETE FROM $$STGDB.WI_SM_NBM_INC_TEMP WHERE BKGS_MEASURE_TRANS_TYPE_CODE='POS' ;
...
CASE WHEN PID_TO_TERM_BNDL IS NULL AND BP.BK_PRODUCT_ID = 'XCAT-ELA-5Y-ENG' THEN  60 ELSE PID_TO_TERM_BNDL END AS PID_TO_TERM_BNDL1,
CASE WHEN PID_TO_TERM_BNDL1>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') THEN NBM.BOOKINGS_PROCESS_DATE END BNDPID_TERM_START_DT,
CASE 
    WHEN PID_TO_TERM_BNDL1>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') 
    THEN NBM.BOOKINGS_PROCESS_DATE +(Cast(PID_TO_TERM_BNDL1 AS DECIMAL(15,6))*365/12)
END BNDPID_TERM_END_DT,
CASE WHEN  PID_TO_TERM_ATTR IS NULL AND P.BK_PRODUCT_ID = 'XCAT-ELA-5Y-ENG' THEN  60 ELSE PID_TO_TERM_ATTR END AS PID_TO_TERM_ATTR1,
CASE WHEN PID_TO_TERM_ATTR1>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') THEN NBM.BOOKINGS_PROCESS_DATE END ATRPID_TERM_START_DT,
CASE WHEN PID_TO_TERM_ATTR1>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') THEN 
NBM.BOOKINGS_PROCESS_DATE +(Cast(PID_TO_TERM_ATTR1 AS DECIMAL(15,6))*365/12)
...
Cast((SOL_CONTRACT_END_DT - SOL_CONTRACT_START_DT+1) AS DECIMAL(15,6)) * 12 /365 AS SOL_CONTRACT_DURATION,

-- SOL Duration Based
CASE 
    WHEN SOLV1.RU_SRVC_CNTRCT_DRTN_MNTHS_CNT <> 0 AND SOLV1.RU_SRVC_CNTRCT_DRTN_MNTHS_CNT <= 240 THEN  SOLV1.RU_SRVC_CNTRCT_DRTN_MNTHS_CNT 
    END AS SOLV1_CONTRACT_DURATION,
CASE 
    WHEN SOLV1_CONTRACT_DURATION>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') 
    THEN NBM.BOOKINGS_PROCESS_DATE 
END SOLV1_CONTRACT_START_DT,
CASE
    WHEN SOLV1_CONTRACT_DURATION>0 AND NBM.BOOKINGS_PROCESS_DATE NOT IN ('1900-01-01' , '3500-01-01') THEN 
...
END DEFAULT_END_DT,
CASE 
WHEN SRVC_CONTRACT_START_DT>'1900-01-01' AND SRVC_CONTRACT_END_DT >'1900-01-01' THEN SRVC_CONTRACT_START_DT
...
CASE 
WHEN SRVC_CONTRACT_START_DT>'1900-01-01' AND SRVC_CONTRACT_END_DT >'1900-01-01' THEN SRVC_CONTRACT_END_DT
...
CASE 
WHEN SRVC_CONTRACT_START_DT>'1900-01-01' AND SRVC_CONTRACT_END_DT >'1900-01-01' THEN SRVC_CONTRACT_DURATION
WHEN SOL_CONTRACT_START_DT>'1900-01-01' AND SOL_CONTRACT_END_DT >'1900-01-01' THEN SOL_CONTRACT_DURATION
WHEN SOLV1_CONTRACT_START_DT>'1900-01-01' AND SOLV1_CONTRACT_END_DT >'1900-01-01' THEN SOLV1_CONTRACT_DURATION
WHEN BKGS_CONTRACT_START_DT>'1900-01-01' AND BKGS_CONTRACT_END_DT >'1900-01-01' THEN BKGS_CONTRACT_DURATION
WHEN BKGS_TERM_START_DT>'1900-01-01' AND BKGS_TERM_END_DT >'1900-01-01' THEN DV_BKGS_TERM_DURATION
WHEN BNDPID_TERM_START_DT>'1900-01-01' AND BNDPID_TERM_END_DT >'1900-01-01' THEN PID_TO_TERM_BNDL1
WHEN ATRPID_TERM_START_DT>'1900-01-01' AND ATRPID_TERM_END_DT >'1900-01-01' THEN PID_TO_TERM_ATTR1
WHEN DEFAULT_START_DT>'1900-01-01' AND DEFAULT_END_DT >'1900-01-01' THEN DEFAULT_TERM
END AS NEW_CONTRACT_DURATION,

CASE 
WHEN SRVC_CONTRACT_START_DT>'1900-01-01' AND SRVC_CONTRACT_END_DT >'1900-01-01' THEN 'SRVC CNTRCT Dates to Term'
...
Cast((NBM.DD_COMP_US_NET_PRICE_AMOUNT / NEW_CONTRACT_DURATION) AS DECIMAL(18,6)) AS NEW_MTHLY_RCR_REV_TRXL_USD_AMT,
Cast((NEW_MTHLY_RCR_REV_TRXL_USD_AMT * 12.000000) AS DECIMAL(18,6)) AS NEW_ANNLZD_MTHY_RCRR_RV_USD_AMT,
INC.AOV_MRR_FLG AS AOV_MRR_FLG
FROM
$$SLSORDVWDB.N_BOOKINGS_MEASURE NBM
...
FROM $$SLSORDVWDB.N_POS_TRANSACTION_LINE WHERE SALES_ORDER_LINE_KEY > 0 GROUP BY 1,2) NS_POS 
ON (NBM.BK_POS_TRANSACTION_ID_INT = NS_POS.BK_POS_TRANSACTION_ID_INT)
LEFT JOIN (
...
ON NBM.BK_POS_TRANSACTION_ID_INT = PNVR.BK_POS_TRANSACTION_ID_INT
...
LEFT JOIN $$SLSORDVWDB.N_SALES_ORDER_LINE_TV SOL ON ( SOL.SALES_ORDER_LINE_KEY = DRVD_SOL_KEY AND SOL.END_TV_DATETIME = '3500-01-01 00:00:00' )
...
LEFT JOIN ( SELECT DISTINCT ITEM_KEY FROM $$COMREFVWDB.MT_CS_DEAL_SVC_FIN_HIER_ALLOC WHERE BK_ALLOCATED_SERVC_GROUP_ID <> 'UNKNOWN' AND DV_PRDT_SUB_GRP_ALLOC_SRC_CD = 'SPA' AND BK_SERVICE_CATEGORY_ID = ('TECHNICAL SUPPORT SERVICES') AND BK_ALLOCATED_SERVC_GROUP_ID NOT IN ('TRAINING')) TSS ON ( NBM.PRODUCT_KEY  = TSS.ITEM_KEY)         
LEFT JOIN (SELECT
...
ON (SVC.SALES_ORDER_LINE_KEY = SOL.SALES_ORDER_LINE_KEY AND ( TSS.ITEM_KEY IS NOT NULL OR  P.TERMS_AND_CONTENT_CD = 'TnC' ))
WHERE 1=1
AND NBM.BKGS_MEASURE_TRANS_TYPE_CODE IN ('POS')
...
NBM_ST.DV_PRODUCT_KEY,
NBM_ST.PRODUCT_KEY,
NBM_ST.DV_ENRICHED_SERVICE_CONTRACT_START_DT, 
NBM_ST.DV_ENRICHED_SERVICE_CONTRACT_END_DT ,
NBM_ST.DV_ENRICHED_CONTRACT_DURATION,
...
NBM_R.TRANSACTION_DATETIME,
NBM_R.BOOKINGS_SPLIT_PCT,
...
CASE WHEN NBM_1.SERVICE_FLG ='Y' THEN 18 ELSE 24 END AS DEFAULT_TERM,
...
AND NBM_1.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
) NBM_R
LEFT JOIN 
(
SELECT NBM_2.*
...
AND NBM_2.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
AND NBM_2.BK_POS_TRANSACTION_ID_INT IN ( SELECT BK_POS_TRANSACTION_ID_INT FROM $$STGDB.WI_NBM_INC WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' GROUP BY 1)
)NBM
ON (-1* NBM.BOOKINGS_MEASURE_KEY = NBM_R.BOOKINGS_MEASURE_KEY
AND NBM.BK_POS_TRANSACTION_ID_INT = NBM_R.BK_POS_TRANSACTION_ID_INT 
)
LEFT JOIN 
(
SELECT NBM_3.*FROM $$SLSORDVWDB.N_BOOKINGS_MEASURE NBM_3
WHERE  NBM_3.FORWARD_REVERSE_FLG = 'F' 
AND NBM_3.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
AND NBM_3.BK_POS_TRANSACTION_ID_INT IN ( SELECT BK_POS_TRANSACTION_ID_INT FROM $$STGDB.WI_NBM_INC WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' GROUP BY 1)
)NBM_F
ON (   NBM.BOOKINGS_MEASURE_KEY IS  NULL AND NBM_F.BK_POS_TRANSACTION_ID_INT =  NBM_R.BK_POS_TRANSACTION_ID_INT
...
LEFT JOIN  (SELECT NBM_4.* FROM $$SLSORDVWDB.N_BOOKINGS_MEASURE NBM_4
WHERE  NBM_4.FORWARD_REVERSE_FLG IN ('F','=') 
AND NBM_4.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
AND NBM_4.BK_POS_TRANSACTION_ID_INT IN ( SELECT BK_POS_TRANSACTION_ID_INT FROM $$STGDB.WI_NBM_INC WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' GROUP BY 1)
)NBM_F_MIS
ON 
(  NBM.DV_ENRICHMENT_RULE_DESC IS  NULL   AND NBM_F.DV_ENRICHMENT_RULE_DESC IS NULL
AND NBM_F_MIS.BK_POS_TRANSACTION_ID_INT =  NBM_R.BK_POS_TRANSACTION_ID_INT AND NBM_F_MIS.BKGS_MEASURE_TRANS_TYPE_CODE =  NBM_R.BKGS_MEASURE_TRANS_TYPE_CODE
...
,NBM.DD_COMP_US_NET_PRICE_AMOUNT                 
 FROM  $$SLSORDVWDB.N_BOOKINGS_MEASURE NBM
JOIN  (SELECT DISTINCT WI.BK_POS_TRANSACTION_ID_INT,WI.DV_PRODUCT_KEY, WI.BOOKINGS_PROCESS_DATE, WI.BKGS_MEASURE_TRANS_TYPE_CODE, WI.FORWARD_REVERSE_FLG,
...
ON NBM.BK_POS_TRANSACTION_ID_INT = SUB.BK_POS_TRANSACTION_ID_INT
...
WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
...
EDW_UPDATE_DATETIME=CURRENT_TIMESTAMP(0)
WHERE NBM.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' AND WI.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
