...
,WDNRXM.RU_SERVICE_CONTRACT_START_DTM
,WDNRXM.RU_SERVICE_CONTRACT_END_DTM
,WDNRXM.DV_CONTRACT_DURATION
,WDNRXM.DV_ANNUALIZED_FLG
,WDNRXM.DV_ANNUALIZED_US_NET_AMT
,WDNRXM.DV_MULTIYEAR_US_NET_AMT
,WDNRXM.BOOKINGS_POLICY_CD
...
,WDNRCG.RU_SERVICE_CONTRACT_START_DTM
,WDNRCG.RU_SERVICE_CONTRACT_END_DTM
,WDNRCG.DV_CONTRACT_DURATION
,WDNRCG.DV_ANNUALIZED_FLG
,WDNRCG.DV_ANNUALIZED_US_NET_AMT
,WDNRCG.DV_MULTIYEAR_US_NET_AMT
,WDNRCG.BOOKINGS_POLICY_CD
...
,BNDL_OFFSET.RU_SERVICE_CONTRACT_START_DTM
,BNDL_OFFSET.RU_SERVICE_CONTRACT_END_DTM
,BNDL_OFFSET.DV_CONTRACT_DURATION
,BNDL_OFFSET.DV_ANNUALIZED_FLG
,BNDL_OFFSET.DV_ANNUALIZED_US_NET_AMT
,BNDL_OFFSET.DV_MULTIYEAR_US_NET_AMT
,BNDL_OFFSET.BOOKINGS_POLICY_CD1 AS BOOKINGS_POLICY_CD
FROM
(SELECT 
...
,A.RU_SERVICE_CONTRACT_START_DTM
,A.RU_SERVICE_CONTRACT_END_DTM
,A.DV_CONTRACT_DURATION
,A.DV_ANNUALIZED_FLG
,A.DV_ANNUALIZED_US_NET_AMT
,A.DV_MULTIYEAR_US_NET_AMT
,CASE WHEN COALESCE(NPT.ACV_FLG,'N')='Y' THEN 'ACV' ELSE 'TCV' END AS BOOKINGS_POLICY_CD1
FROM  
...
,A.DV_CONTRACT_DURATION
,A.DV_ANNUALIZED_FLG
,A.DV_ANNUALIZED_US_NET_AMT*-1
,A.DV_MULTIYEAR_US_NET_AMT*-1
,CASE WHEN COALESCE(NPT.ACV_FLG,'N')='Y' THEN 'ACV' ELSE 'TCV' END AS BOOKINGS_POLICY_CD1
FROM  
$$STGDB.WI_DRVD_NCR_REV_FOR_BKG A  
LEFT OUTER JOIN  $$COMREFVWDB.N_PRODUCT NPT
...
,WDNRFB.RU_SERVICE_CONTRACT_START_DTM
,WDNRFB.RU_SERVICE_CONTRACT_END_DTM
,WDNRFB.DV_CONTRACT_DURATION
,WDNRFB.DV_ANNUALIZED_FLG
,WDNRFB.DV_ANNUALIZED_US_NET_AMT
,WDNRFB.DV_MULTIYEAR_US_NET_AMT
,WDNRFB.BOOKINGS_POLICY_CD
,WDNRFB.ACTION_CODE
,WDNRFB.DML_TYPE 
FROM 
 ...
 (  SELECT 1 FROM 
	  (
		 ...
		 SELECT BK_TRANSACTION_SEQ_ID_INT AS TRANSACTION_SEQ_ID_INT, BK_SS_CD AS SS_CD FROM $$STGDB.WI_BKG_TX_SQ_ID_NON_APLD_SC_UD
		  UNION
		SELECT TRANSACTION_SEQUENCE_ID_INT,'CG' AS SS_CD FROM $$STGDB.WI_CONTRACT_CHANGE_AR_FWD /*Annuity net change*/
	  ) WI  
     WHERE WI.TRANSACTION_SEQ_ID_INT = DRVD.TRANSACTION_SEQUENCE_ID_INT
      AND WI.SS_CD = DRVD.SOURCE_SYSTEM_CODE
   )
AND 1=2 /* Added as part of SQ Net change removal - Day1 FY21 */
...
,DRVD.DV_CONTRACT_DURATION
,DRVD.DV_ANNUALIZED_FLG
,DRVD.DV_ANNUALIZED_US_NET_AMT*-1 AS DV_ANNUALIZED_US_NET_AMT
,DRVD.DV_MULTIYEAR_US_NET_AMT* -1 AS DV_MULTIYEAR_US_NET_AMT
,DRVD.BOOKINGS_POLICY_CD
FROM $$FINLGLVWDB.N_DRVD_NCR_REV_TRX_FOR_BKG DRVD 
INNER JOIN 
    ( SELECT SALES_ORDER_LINE_KEY, SALES_MOTION_CD, SOURCE_COMMIT_TIME
...
SELECT 
	...
	,WI.RU_SERVICE_CONTRACT_START_DTM
	,WI.RU_SERVICE_CONTRACT_END_DTM
	,WI.DV_CONTRACT_DURATION
	,WI.DV_ANNUALIZED_FLG
	,WI.DV_ANNUALIZED_US_NET_AMT
	,WI.DV_MULTIYEAR_US_NET_AMT
	,WI.BOOKINGS_POLICY_CD
...
/*BOOKINGS_POLICY_CD UPDATES. IF RECORDS DIDN'T FALL IN THIS CATEGORY WILL TAG AS TCV
...
SET BOOKINGS_POLICY_CD=COALESCE(NSOL.BOOKINGS_POLICY_CD  ,'TCV')          
WHERE WK.SALES_ORDER_LINE_KEY=NSOL.SALES_ORDER_LINE_KEY
AND WK.PRODUCT_KEY=NSOL.PRODUCT_KEY   
AND NSOL.SS_CODE NOT IN ('OPL')
AND WK.DV_SALES_ORDER_LINE_KEY>0 AND WK.CREATED_BY_INT=1;

/* APPLIED ERP SUBSKU*/

UPDATE WK FROM 
$$WORKDB.W_DRVD_NCR_REV_TRX_FOR_BKG WK,
(SELECT TOP_SKU_SALES_ORDER_LINE_KEY,ATTRIBUTED_PRODUCT_KEY,BOOKINGS_POLICY_CD FROM  $$NRTNCRVWDB.N_SOL_OFFER_ATTRIBUTION 
 GROUP BY 1,2,3) NSOL_OA
SET BOOKINGS_POLICY_CD=COALESCE (NSOL_OA.BOOKINGS_POLICY_CD ,'TCV')            
WHERE WK.DV_SALES_ORDER_LINE_KEY=NSOL_OA.TOP_SKU_SALES_ORDER_LINE_KEY
AND WK.PRODUCT_KEY=NSOL_OA.ATTRIBUTED_PRODUCT_KEY   
AND WK.DV_SALES_ORDER_LINE_KEY>0 
AND WK.CREATED_BY_INT=1 AND WK.DV_ATTRIBUTION_CD IN ('ATTRIBUTED');

/*APPLIED XAAS TOPSKU (REGULAR)  */

UPDATE WK FROM 
$$WORKDB.W_DRVD_NCR_REV_TRX_FOR_BKG WK,
$$FINLGLVWDB.N_AR_TRX_LINE NATL,
(SELECT SBSCRPTN_ITEM_CHANGE_ID_INT,SUBSCRIPTION_PRODUCT_KEY,BOOKINGS_POLICY_CD FROM $$SLSORDVWDB.N_XAAS_SO_SBSCR_ITM_SLS_TRX
GROUP BY 1,2,3) NXAAS
SET BOOKINGS_POLICY_CD=COALESCE (NXAAS.BOOKINGS_POLICY_CD  ,'TCV')           
WHERE WK.DV_AR_TRX_LINE_KEY=NATL.AR_TRX_LINE_KEY
AND NATL.SBSCRPTN_ITEM_CHANGE_ID_INT=NXAAS.SBSCRPTN_ITEM_CHANGE_ID_INT
AND WK.PRODUCT_KEY=NXAAS.SUBSCRIPTION_PRODUCT_KEY   
AND NATL.SBSCRPTN_ITEM_CHANGE_ID_INT>0 AND WK.CREATED_BY_INT=1;