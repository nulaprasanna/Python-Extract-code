...
AND NBM.SK_SALES_MOTION_ATTRIB_KEY < 0; /* Added to Qualify Bundle and Offset Records */
...
Coalesce(NBM_F.TRANSACTION_DATETIME, NBM.TRANSACTION_DATETIME , NBM_F_MIS.BOOKINGS_PROCESS_DATE) AS REF_TRANSACTION_DATETIME,
...
FROM (
SELECT NBM.*,NBM.BOOKINGS_PROCESS_DATE AS DEFAULT_CONTRACT_ST_DT,
CASE WHEN NBM.SERVICE_FLG ='Y' THEN 18 ELSE 24 END AS DEFAULT_TERM,
...
ON ( INC.BOOKINGS_MEASURE_KEY = NBM.BOOKINGS_MEASURE_KEY 
      AND INC.BOOKINGS_PROCESS_DATE = NBM.BOOKINGS_PROCESS_DATE 
      AND INC.BKGS_MEASURE_TRANS_TYPE_CODE = NBM.BKGS_MEASURE_TRANS_TYPE_CODE)
WHERE NBM.FORWARD_REVERSE_FLG = 'R' AND INC.FORWARD_REVERSE_FLG ='R'
AND NBM.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
) NBM_R
LEFT JOIN 
(
SELECT NBM.*
FROM $$SLSORDVWDB.N_BOOKINGS_MEASURE NBM
WHERE  NBM.FORWARD_REVERSE_FLG = 'F' 
AND NBM.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
AND NBM.BK_POS_TRANSACTION_ID_INT IN ( SELECT BK_POS_TRANSACTION_ID_INT FROM $$WORKDB.W_BOOKINGS_MEASURE WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' GROUP BY 1)
)NBM
...
LEFT JOIN  (SELECT NBM.* FROM $$SLSORDVWDB.N_BOOKINGS_MEASURE NBM
WHERE  NBM.FORWARD_REVERSE_FLG IN ('F','=') 
AND NBM.BKGS_MEASURE_TRANS_TYPE_CODE = 'POS'
AND NBM.BK_POS_TRANSACTION_ID_INT IN ( SELECT BK_POS_TRANSACTION_ID_INT FROM $$WORKDB.W_BOOKINGS_MEASURE WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'POS' GROUP BY 1)
)NBM_F_MIS
ON 
