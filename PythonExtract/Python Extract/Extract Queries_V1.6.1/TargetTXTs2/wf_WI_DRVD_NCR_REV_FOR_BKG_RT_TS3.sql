...
/*WIDRVD.SALES_TERRITORY_KEY           ,*//*commented as part of AR default sc logic change q1fy21*/
CASE WHEN (NABS.BK_BATCH_SOURCE_NAME IN ('DCA TWO-TIER','DCA TWO TIER') AND WIDRVD.SALES_TERRITORY_KEY=73106)  /*No identifier to identify default SC*/
     THEN RSH.SALES_TERRITORY_KEY ELSE WIDRVD.SALES_TERRITORY_KEY END AS SALES_TERRITORY_KEY,
...
CASE WHEN NRT.TRANSACTION_SOURCE_CD='ERP' 
     THEN (CASE WHEN NSOLN.RU_SERVICE_CONTRACT_START_DTM IN ('1900-01-01 00:00:00','3500-01-01 00:00:00') THEN NULL ELSE NSOLN.RU_SERVICE_CONTRACT_START_DTM END)
	 ELSE (CASE WHEN NSOLN_XAAS.RU_SERVICE_CONTRACT_START_DTM IN ('1900-01-01 00:00:00','3500-01-01 00:00:00') THEN NULL ELSE NSOLN_XAAS.RU_SERVICE_CONTRACT_START_DTM END)
END AS RU_SERVICE_CONTRACT_START_DTM1,
CASE WHEN NRT.TRANSACTION_SOURCE_CD='ERP' 
     THEN (CASE WHEN NSOLN.RU_SERVICE_CONTRACT_END_DTM IN ('1900-01-01 00:00:00','3500-01-01 00:00:00') THEN NULL ELSE NSOLN.RU_SERVICE_CONTRACT_END_DTM END)
	 ELSE (CASE WHEN NSOLN_XAAS.RU_SERVICE_CONTRACT_END_DTM IN ('1900-01-01 00:00:00','3500-01-01 00:00:00') THEN NULL ELSE NSOLN_XAAS.RU_SERVICE_CONTRACT_END_DTM END)
END AS RU_SERVICE_CONTRACT_END_DTM1,
CASE WHEN NRT.TRANSACTION_SOURCE_CD='ERP' THEN NSOLV.RU_SRVC_CNTRCT_DRTN_MNTHS_CNT ELSE NSOLV_XAAS.RU_SRVC_CNTRCT_DRTN_MNTHS_CNT END AS RU_SRVC_CNTRCT_DRTN_MNTHS_CNT
 FROM
$$STGDB.WI_DRVD_NCR_REV_TRX_RT  WIDRVD
/*LEFT OUTER JOIN $$STGDB.WI_AR_SALES_MOTION_CHANGE SMC
ON WIDRVD.ORDER_LINE_ID = SMC.SK_SO_LINE_ID_INT*/ /* Commented as part of SQ Net change removal - Day1 FY21 */
  /*acv changes*/
  INNER JOIN $$FINLGLVWDB.N_REVENUE_TRANSFER NRT
  ON WIDRVD.DV_TRANSACTION_KEY=NRT.REVENUE_TRANSFER_KEY
  LEFT OUTER JOIN $$ETLVWDB.SM_SALES_ORDER_LINE SSOL
   ON ( SSOL.SK_SO_LINE_ID_INT = WIDRVD.ORDER_LINE_ID 
        AND SSOL.SS_CODE = 'CG')
   ...
   AND NSOLV.END_TV_DTM = '3500-01-01 00:00:00')  
   /*SC DEFAULT LOGIC FOR DCA 2-TIER*/
...
SELECT
 ...
 ELSE COALESCE(EST_REVERSE.TERRITORY_ID, -999) END as TERRITORY_ID ,
  ...
  WI_NET.RU_SERVICE_CONTRACT_START_DTM AS RU_SERVICE_CONTRACT_START_DTM,
  WI_NET.RU_SERVICE_CONTRACT_END_DTM AS RU_SERVICE_CONTRACT_END_DTM,
  WI_NET.DV_CONTRACT_DURATION AS DV_CONTRACT_DURATION,
...
SELECT 
 ...
 ROUND(CASE WHEN WDNRTFB.FORWARD_REVERSE_CODE IN ('R','REVERSE') THEN WIOXRT.DV_CONTRACT_DURATION  
               ELSE 
               CASE WHEN WDNRTFB.DV_BKGS_SERVICE_FLAG= 'N' THEN 
			        ROUND(CASE --WIOXRT.RU_SERVICE_CONTRACT_START_DTM <> CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)) AND WIOXRT.RU_SERVICE_CONTRACT_END_DTM <> CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)) AND ( ( CAST(WIOXRT.RU_SERVICE_CONTRACT_END_DTM  AS DATE ) - CAST(WIOXRT.RU_SERVICE_CONTRACT_START_DTM AS DATE ) )  > 0) 
							   ...
							   CASE WHEN NP.PRICING_UNIT_NAME = 'YEAR' THEN NP.PRICING_UNIT_DURATION_INT * 12 
									...
									WHEN NP.PRICING_UNIT_NAME = 'DAYS' THEN CAST(((NP.PRICING_UNIT_DURATION_INT *12)/365) AS DECIMAL(15,6))
			    ...
			    CASE --WHEN WIOXRT.RU_SERVICE_CONTRACT_START_DTM <> CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)) AND WIOXRT.RU_SERVICE_CONTRACT_END_DTM <> CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)) AND ( ( CAST(WIOXRT.RU_SERVICE_CONTRACT_END_DTM  AS DATE ) - CAST(WIOXRT.RU_SERVICE_CONTRACT_START_DTM AS DATE ) )  > 0)  
                     ...
                     WHEN  WIOXRT.DV_CONTRACT_DURATION > 0 THEN WIOXRT.DV_CONTRACT_DURATION
                END 
  END END) AS DV_CONTRACT_DURATION1, /*REMOVED COALESCE TO PREVENT UNNECESSARY NET CHANGES*/
  
 COALESCE(CASE WHEN WDNRTFB.FORWARD_REVERSE_CODE IN ('R','REVERSE') THEN WIOXRT.DV_ANNUALIZED_FLG 
     ELSE   
     CASE WHEN WDNRTFB.DV_BKGS_SERVICE_FLAG= 'N' THEN 
...
CASE WHEN WDNRTFB.FORWARD_REVERSE_CODE IN ('R','REVERSE') THEN -1* WIOXRT.DV_ANNUALIZED_US_NET_AMT
     ELSE 
...
CASE WHEN WDNRTFB.FORWARD_REVERSE_CODE IN ('R','REVERSE') THEN -1* WIOXRT.DV_MULTIYEAR_US_NET_AMT
     ELSE 
...
/*UPDATING SO KEY HERE AS IT IS REFERRED IN DERIVATION OF ANNUALIZED_FLG*/

UPDATE WI FROM 
$$STGDB.WI_DRVD_NCR_REV_TRX_FOR_BKG_RT WI,
$$STGDB.WI_OM_XXNCR_REV_RT WI_OM,
$$FINLGLVWDB.N_REVENUE_TRANSFER NRT,
$$FINLGLVWDB.N_AR_TRX_ITEM_LINE NATIL
SET SALES_ORDER_KEY=NATIL.EXAAS_SUBSCR_SO_KEY
WHERE WI.TRANSACTION_SEQUENCE_ID_INT=WI_OM.TRANSACTION_SEQUENCE_ID   
AND WI_OM.DV_TRANSACTION_KEY=NRT.REVENUE_TRANSFER_KEY
AND NRT.TRANSACTION_SOURCE_CD='XAAS'
AND NRT.AR_TRANSACTION_LINE_KEY=NATIL.AR_TRX_LINE_KEY;
...
/*code to update contract duariton to null incase of -999*/
UPDATE WREV FROM 
$$STGDB.WI_DRVD_NCR_REV_FOR_BKG_RT WREV
