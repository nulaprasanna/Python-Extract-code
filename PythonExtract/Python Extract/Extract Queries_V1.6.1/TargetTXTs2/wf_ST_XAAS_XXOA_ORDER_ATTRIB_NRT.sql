...
SELECT 
  ...
  CXOA.ATTRIBUTION_DETAILS      ,
    ...
    CXOA.RECOGNIZED_ATTR_SALES_VALUE AS RECOGNIZED_ATTR_SALES_VALUES,
    CXOA.RECOGNIZED_ATTR_TCV,
    CXOA.BOOKING_VERSION,
    CXOA.ERP_LINE_ID,
    CXOA.ERP_PARENT_LINE_ID,
    cast(cast(CREATION_DATE  as varchar(19)) as timestamp(0)),
    cast(cast(EDW_CREATE_DTM as varchar(19)) as timestamp(0)) AS EDW_CREATE_DTM1,  
    ACV_ATTRIBUTION_PCT,
	CXOA.PRICING_TERM         /* SIGORANT ADDED AS PART OF Q2FY19 */
   /*ADDED AS PART OF Q1FY21 ACV SCOPE*/
   ,CXOA.ATTRIB_ACV_FLAG AS ACV_FLAG
   ,CXOA.ATTRIB_ACV_AMOUNT AS ACV_AMOUNT
   ,CXOA.ITEM_ACV_ELIGIBLE_FLAG AS ACV_ELIGIBLE_FLAG
    /*END OF Q1FY21 ACV SCOPE*/
    FROM
    $$STGDB.ST_SI_XXOA_ORDER_ATTRIB CXOA
    QUALIFY ROW_NUMBER() OVER (PARTITION BY CXOA.ATTRIBUTION_ID ORDER BY CXOA.EDW_CREATE_DTM DESC)=1
    WHERE   
    --SOURCE_DML_TYPE IN ( 'SQL COMPUPDATE','INSERT')
     --AND PUBLISH_FLAG='Y'
  (DATA_SOURCE IN ('TSL','NGCCRM') OR (DATA_SOURCE IN ('OPL') AND ATTRIBUTION_TYPE IN ('BOOKING'))
  
  OR (DATA_SOURCE IN ('OPL') AND ATTRIBUTION_TYPE IN ('ORDER') AND COALESCE(TRX_ID,PARENT_TRX_ID) IS NOT NULL)
  ) 
     --AND TRX_USAGE_CODE='R' AND  TRX_TYPE_CODE='B'
     AND EDW_CREATE_DTM1  >= '$$LastExtractDate'
     ...
     AND EDW_CREATE_DTM1  <=(  /*Incremental Extraction based on 9 TO 9 Logic*/
         SELECT 
          CASE 
