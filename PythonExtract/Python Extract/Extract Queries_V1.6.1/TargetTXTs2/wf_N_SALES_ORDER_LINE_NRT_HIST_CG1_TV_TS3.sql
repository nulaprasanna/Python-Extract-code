...
/* Added as part of Q1FY21 ACV*/
WSOLNH.TRANSACTIONAL_ACV_FLG,
WSOLNH.ACV_FLG,
WSOLNH.ANNUAL_TRANSACTIONAL_AMT
/*END OF ACV SCALE*/
FROM
$$NRTSTGDB.W_SALES_ORDER_LINE_NRT_HIST WSOLNH   
...
WHERE NSOLNT.SALES_ORDER_LINE_KEY = WI.SALES_ORDER_LINE_KEY 
AND   NSOLNT.END_TV_DTM='3500-01-01 00:00:00'
AND   (   COALESCE(NSOLNT.RU_SERVICE_CONTRACT_START_DTM,CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)))
	   <> COALESCE(WI.RU_SERVICE_CONTRACT_START_DTM,CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)))
	   OR  COALESCE(NSOLNT.RU_SERVICE_CONTRACT_END_DTM,CAST('3500/01/01 00:00:00' AS TIMESTAMP(0)))
...
WI_ACV.SOURCE_COMMIT_DTM
FROM 
(SEL * FROM $$NRTSTGDB.WI_ORDER_LINES_EXT_NRT_CG1_PBL
...
WHERE NSOLNT.SALES_ORDER_LINE_KEY = WI.SALES_ORDER_LINE_KEY 
AND   NSOLNT.END_TV_DTM='3500-01-01 00:00:00'
AND   (    NSOLNT.ANNUAL_TRANSACTIONAL_AMT <> WI.ANNUAL_TRANSACTIONAL_AMT
	   OR  NSOLNT.TRANSACTIONAL_ACV_FLG    <> WI.TRANSACTIONAL_ACV_FLG
       OR  NSOLNT.ACV_FLG                  <> WI.ACV_FLG
	   )
;
...
FROM $$NRTNCRVWDB.N_SALES_ORDER_LINE_NRT_HIST_TV NSOLNT, 
 ( SELECT SALES_ORDER_LINE_KEY, 
	...
	/* AND SALES_ORDER_LINE_KEY NOT IN ( SELECT SALES_ORDER_LINE_KEY 
									...
									GROUP BY 1 HAVING ROUND(SUM(DV_ALLOCATION_PCT)) = 1.00) */ /* Commented above as part of SQ Net change removal - Day1 FY21 */
	QUALIFY ROW_NUMBER() OVER( PARTITION BY LINE_ID ORDER BY SOURCE_COMMIT_DTM DESC) = 1 
	) WI
SET SALES_MOTION_CD = WI.SALES_MOTION_CD , 
    SOURCE_COMMIT_DTM = WI.SOURCE_COMMIT_DTM,
