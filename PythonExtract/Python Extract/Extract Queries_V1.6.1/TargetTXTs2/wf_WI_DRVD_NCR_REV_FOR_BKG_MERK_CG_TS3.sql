...
Source1 Name : SQ_WI_DRVD_NCR_REV_BKG_MERK_CG


Pre SQL : 
DELETE FROM $$STGDB.WI_DRVD_NCR_REV_BKG_MERK_CG ALL;
...
SELECT 
 ...
 COALESCE( SMC.SALES_MOTION_CD, 'UNKNOWN' ) SALES_MOTION_CD /*Added as part of Oct-2017 release */
  FROM
   $$STGDB.WI_DRVD_NCR_REV_MERK_TRX_CG WI
   INNER JOIN $$FINLGLVWDB.N_MERAKI_REVENUE_TRX NXOA 
     ON NXOA.MERAKI_REV_TRX_KEY = WI.DV_TRANSACTION_KEY
 LEFT OUTER JOIN $$STGDB.WI_AR_SALES_MOTION_CHANGE SMC    
 ON WI.ORDER_LINE_ID = SMC.SK_SO_LINE_ID_INT
...
Target1 Name : WI_DRVD_NCR_REV_BKG_MERK_CG2
...
SELECT
 ...
 WI_NET.PD_FUNCTIONAL_CURRENCY_CODE AS  FUNC_CURRENCY_CODE ,
  ...
  UNION ALL
    ...
    /*Meraki Changes*/     
    UNION ALL
    SELECT MAX(TRANSACTION_SEQUENCE_ID   ) AS TRANSACTION_SEQUENCE_ID_INT FROM $$STGDB.WI_DRVD_NCR_REV_MERK_TRX_CG  /*Meraki Changes*/
      UNION ALL
    SELECT MAX(TRANSACTION_SEQUENCE_ID   ) AS TRANSACTION_SEQUENCE_ID_INT FROM $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG 
    UNION ALL
    SELECT MAX(TRANSACTION_SEQUENCE_ID   ) AS TRANSACTION_SEQUENCE_ID_INT FROM $$EXCEPDB.EX_OM_XXNCR_REV_MERK_CG 
...
/* As part of Net Accounting*/
  ...
  INNER JOIN $$FINLGLVWDB.N_MERAKI_REVENUE_TRX NXOA  
   ON WI_NET.MERAKI_REV_TRX_KEY = NXOA.MERAKI_REV_TRX_KEY
  ...
  AND WI_NET.MERAKI_REV_TRX_KEY>0
...
Target2 Name : WI_DRVD_NCR_REV_BKG_MERK_CG_net_chg_rev_inv
...
Post SQL : 
CALL COLLECT_STATS_WRAP('$$STGDB','WI_DRVD_NCR_REV_BKG_MERK_CG','D');


Source3 Name : SQ_WI_OM_XXNCR_REV_MERK_CG_P_2


Pre SQL : 
DELETE FROM $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG EXORT
  ...
  ( SELECT STOXRT.GLOBAL_NAME ,STOXRT.DV_TRANSACTION_KEY, STOXRT.DV_TRANSACTION_SOURCE_CD,STOXRT.CUSTOMER_TRX_LINE_ID,STOXRT.FISCAL_ID
   FROM $$STGDB.WI_DRVD_NCR_REV_MERK_TRX_CG STOXRT
   );
   
  DELETE FROM $$EXCEPDB.EX_OM_XXNCR_REV_MERK_CG EXORT
  ...
  FROM
   ...
   $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG EX ,
  ...
  DELETE FROM $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG EX 
  WHERE EX.EXCEPTION_TYPE   = 'FT'
  AND EX.FISCAL_ID  IN ( SELECT FISCAL_YEAR_MTH_NUMBER_INT FROM $$ETLVWDB.BKG_PROCESS_DT_CNTRL);
   
   
  COLLECT STATS ON $$STGDB.WI_DRVD_NCR_REV_MERK_TRX_CG;
  
  DELETE FROM $$STGDB.WI_OM_XXNCR_REV_MERK_CG ALL;
...
SELECT 
   ...
   CASE WHEN NRA.ATTRIBUTION_CD='OFFSET'
   ...
   FROM 
    $$STGDB.WI_DRVD_NCR_REV_BKG_MERK_CG STOXRT
  INNER JOIN $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
    ON ( NSSC.GLOBAL_NAME = STOXRT.GLOBAL_NAME)     
  INNER JOIN $$FINLGLVWDB.N_MERAKI_REVENUE_TRX NRA
  ON NRA.MERAKI_REV_TRX_KEY=STOXRT.DV_TRANSACTION_KEY
  INNER JOIN $$FINLGLVWDB.N_AR_TRX_LINE NATL
  ...
  ON (NPT_PAR.ITEM_KEY = NRA.DV_BUNDLE_PRDT_KEY)  
   ...
   (
    ...
    AND (
	  ...
	  OR (COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT) = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY')   /*Added AND : TNC Feb 2020 Release*/ )
        OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE  'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT LIKE '%XAAS%' /*Added OR : TNC Feb 2020 Release*/ )
       )
   ...
   AND 
	  ...
	  (COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT) = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY')   /*Added AND : TNC Feb 2020 Release*/ )
        OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE  'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT LIKE '%XAAS%' /*Added OR : TNC Feb 2020 Release*/ )
       )
  ...
  OR (NATT.AR_TRX_TYPE_SHORT_CODE  IN ('DM' ,'CM','INV') AND STOXRT.REQUEST_ID <> -999
	  ...
	  OR ( COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT) IN ('ICMS-ADJ') AND NATL.TRX_LINE_SRC_NAME IN ('XAAS')   
	 ...
	 OR (NABS.BK_BATCH_SOURCE_NAME IN ('ICMS-ADJ') AND NATL.TRX_LINE_SRC_NAME LIKE 'XAAS%' )   /*Added OR : TNC Feb 2020 Release*/ )
  ...
  OR (NATT.AR_TRX_TYPE_SHORT_CODE  IN ('DM' ,'CM','INV') AND STOXRT.REQUEST_ID <> -999
      AND( (COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT)  = 'ORDER ENTRY' 
	  OR (COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT) = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY')   /*Added AND : TNC Feb 2020 Release*/ )
        OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE  'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT IN ('XAAS')) /*Added OR : TNC Feb 2020 Release*/ 
        )
      AND CASE WHEN NSOL_NRT.BOOKINGS_SOURCE_CD <> 'UNK' THEN NSOL_NRT.BOOKINGS_SOURCE_CD
                WHEN NPT_PAR.RU_BKG_SRC_CD <> 'UNK' THEN NPT_PAR.RU_BKG_SRC_CD
             ELSE 'UNK' END  = 'TSL'
...
UNION ALL 
 ...
 STOXRT.SALES_MOTION_CD /* Added as part of Oct-2017 rel */  
   FROM 
    $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG STOXRT
   INNER JOIN $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
    ON ( NSSC.GLOBAL_NAME = STOXRT.GLOBAL_NAME)    
    INNER JOIN $$FINLGLVWDB.N_MERAKI_REVENUE_TRX NRA
  ON NRA.MERAKI_REV_TRX_KEY=STOXRT.DV_TRANSACTION_KEY
  ...
  ON (NPT_PAR.ITEM_KEY = NRA.DV_BUNDLE_PRDT_KEY)  
   ...
   CASE WHEN NSOL_NRT.BOOKINGS_SOURCE_CD <> 'UNK' THEN NSOL_NRT.BOOKINGS_SOURCE_CD
     ...
     OR (NATT.AR_TRX_TYPE_SHORT_CODE  IN ('DM' ,'CM','INV') AND STOXRT.REQUEST_ID <> -999
      AND ((COALESCE(EORCTL.INTERFACE_LINE_CONTEXT ,EORCT.INTERFACE_HEADER_CONTEXT) 
	  IN ('SALES ADJUSTMENTS','DISPUTE','DCA TWO-TIER' ,'COOP TWO-TIER','ICMS-ADJ','DCA TWO TIER','COOP TWO TIER')
	  AND NATL.TRX_LINE_SRC_NAME NOT IN ('XAAS')   /*Added OR : TNC Feb 2020 Release*/ )
       OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) IN ('DISPUTE','ICMS-ADJ') AND NATL.TRX_LINE_SRC_NAME NOT IN ('XAAS'))
      )
      AND 
   CASE WHEN NSOL_NRT.BOOKINGS_SOURCE_CD <> 'UNK' THEN NSOL_NRT.BOOKINGS_SOURCE_CD
 ...
 EXORT.SALES_MOTION_CD /* Added as part of Oct-2017 rel */  
   FROM 
   $$EXCEPDB.EX_OM_XXNCR_REV_MERK_CG EXORT
...
Target3 Name : WI_OM_XXNCR_REV_MERK_CG1
...
Source4 Name : SQ_EX_WI_OM_XXNCR_REV_MERK_CG_P_3
...
SELECT 
 ...
 STOXRT.SALES_MOTION_CD /* Added as part of Oct-2017 rel */ 
  ...
  WHERE 
    ...
    AND  NOT EXISTS
    ( SELECT 1 FROM   $$STGDB.WI_OM_XXNCR_REV_MERK_CG WI 
...
Target4 Name : EX_WI_OM_XXNCR_REV_MERK_CG1
...
/* RCHETTRI : 
  ...
  */
  INSERT INTO  $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG
  (
  BATCH_ID                       ,
  ACCOUNTING_RULE_ID             ,
  ...
  AND NOT EXISTS ( SELECT  1 FROM $$EXCEPDB.EX_WI_OM_XXNCR_REV_MERK_CG EX
      WHERE EX.GLOBAL_NAME = STOXRT.GLOBAL_NAME
      AND EX.TRANSACTION_SEQUENCE_ID = STOXRT.TRANSACTION_SEQUENCE_ID
      ) ;
  
  
  CALL COLLECT_STATS_WRAP('$$EXCEPDB','EX_WI_OM_XXNCR_REV_MERK_CG','D');


Source5 Name : SQ_WI_DRVD_NCR_SLS_CHL_CD_MERK_CG_P_4
...
SELECT 
  ...
  FROM 
   $$STGDB.WI_OM_XXNCR_REV_MERK_CG STOXREVTRX
  INNER JOIN  $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
  ...
  FROM 
   $$STGDB.WI_OM_XXNCR_REV_MERK_CG WIOXRT
  INNER JOIN $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
...
Target5 Name : WI_DRVD_NCR_SLS_CHL_CD_MERK_CG1


Pre SQL : 
DELETE FROM $$STGDB.WI_DRVD_NCR_SLS_CHL_CD_MERK_CG ALL;


Post SQL : 
CALL COLLECT_STATS_WRAP('$$STGDB','WI_DRVD_NCR_SLS_CHL_CD_MERK_CG','D');


Source6 Name : SQ_WI_DRVD_NCR_CMP_US_CST_MERK_CG_P_5
...
SELECT 
  ...
  INNER JOIN (SELECT 
   ...
   FROM
   $$STGDB.WI_OM_XXNCR_REV_MERK_CG WIOXRT1
   INNER JOIN $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
...
Target6 Name : WI_DRVD_NCR_CMP_US_CST_MERK_CG1


Pre SQL : 
DELETE FROM $$STGDB.WI_DRVD_NCR_CMP_US_CST_MERK_CG ALL;


Post SQL : 
CALL COLLECT_STATS_WRAP('$$STGDB','WI_DRVD_NCR_CMP_US_CST_MERK_CG','D');


Source7 Name : SQ_WI_DRVD_NCR_REV_TRX_FOR_BKG_MERK_CG_P_6
...
SELECT
    ...
    CASE WHEN NRA.ATTRIBUTION_CD='OFFSET'
         THEN  (-1)* (NRA.CREDIT_TRXL_AMT+NRA.DEBIT_TRXL_AMT)
      ELSE (NRA.CREDIT_TRXL_AMT+NRA.DEBIT_TRXL_AMT)
    ...
    CASE WHEN ((EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY' OR (EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY' )  /*Added AND : TNC Feb 2020 Release*/ ) OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE 'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT LIKE '%XAAS%' /*Added AND : TNC Feb 2020 Release*/ ))
        AND (NSOT.BK_ORDER_TYPE_NAME ) like '%RMA%' 
        AND UPPER(NSORDTV.SALES_ORDER_CATEGORY_TYPE) <> 'MIXED'
       THEN WIOXRT.TRANSACTION_QUANTITY 
      WHEN ((EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY' OR (EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY' )  /*Added AND : TNC Feb 2020 Release*/ ) OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE 'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT LIKE '%XAAS%' /*Added AND : TNC Feb 2020 Release*/ ))
    ...
    CASE WHEN NRA.ATTRIBUTION_CD='OFFSET' 
      ...
      ELSE CAST ((NRA.OFFR_ATTR_PCT/100.000000)*(NATIL.RU_CREDIT_UNIT_AMOUNT + NATIL.RU_DEBIT_UNIT_AMOUNT) AS DECIMAL(18,6))
    END UNIT_SELLING_PRICE_TRX_AMT,
    CASE WHEN NRA.ATTRIBUTION_CD='OFFSET' 
         THEN (-1)*(NATIL.RU_UNIT_STD_PRICE_LOCAL_AMT)
      ELSE CAST ((NRA.OFFR_ATTR_PCT/100.000000)* (NATIL.RU_UNIT_STD_PRICE_LOCAL_AMT) AS DECIMAL(18,6)) 
  ...
  CASE WHEN NSOT.SO_TYPE_REVENUE_GEN_FLAG = '=' OR NSOT.SO_TYPE_REVENUE_GEN_FLAG IS NULL THEN 'Y' ELSE NSOT.SO_TYPE_REVENUE_GEN_FLAG END as DV_BKGS_REVENUE_FLAG,  /*     added as part of Cf LSS CR post Q3FY15: If Order got created in old 11i system then flag will be derived based on order table otherwise if order gets created in new CG system, then flag will be derived from order line table*/
    ...
    CASE WHEN WIOXRT.EXTRACT_TYPE='CORP_ADJ' THEN 0 
      ELSE  ( CASE WHEN NPT.GOODS_OR_SERVICE_TYPE='SERVICE' THEN 0 
           ELSE CAST( (CASE WHEN NRA.ATTRIBUTION_CD='OFFSET' THEN ( NPPT.LIST_PRICE_AMOUNT) ELSE (NRA.OFFR_ATTR_PCT*NPPT.LIST_PRICE_AMOUNT/100.000000) END *
   ...
   0 DV_BKGS_COMP_US_COST_AMT,
    ...
    CASE WHEN NCCDR.PL_CONVERSION_RATE  =0 OR NCCDR.PL_CONVERSION_RATE  IS NULL THEN 1 ELSE NCCDR.PL_CONVERSION_RATE  END CONVERSION_RT,
      CASE WHEN ((EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY' OR (EORCTL.INTERFACE_LINE_CONTEXT = 'ORDER ENTRY TNC' AND NATL.TRX_LINE_SRC_NAME = 'ORDER ENTRY' )  /*Added AND : TNC Feb 2020 Release*/ ) OR (UPPER(NABS.BK_BATCH_SOURCE_NAME) LIKE 'ORDER MANAGEMENT%' AND NATL.TRX_LINE_SRC_NAME NOT LIKE '%XAAS%' /*Added AND : TNC Feb 2020 Release*/ ))  AND (NSOT.BK_ORDER_TYPE_NAME) like '%RMA%' 
 AND NATT.AR_TRX_TYPE_SHORT_CODE = 'CM' 
 THEN 
  CAST(CASE WHEN NRA.ATTRIBUTION_CD='OFFSET' THEN ( NSOLN.UNIT_LIST_PRICE) ELSE (NRA.OFFR_ATTR_PCT*NSOLN.UNIT_LIST_PRICE/100.000000) END 
 ...
 CASE WHEN WIOXRT.EXTRACT_TYPE IN ('INV' ,'REV')
         THEN CAST(
   CAST((CASE WHEN NRA.ATTRIBUTION_CD='OFFSET' THEN (-1) ELSE (NRA.OFFR_ATTR_PCT/100.000000) END) AS DECIMAL(26,7)) *
 ...
 FROM 
    $$STGDB.WI_OM_XXNCR_REV_MERK_CG WIOXRT
     ...
     ON (NSSC.GLOBAL_NAME = WIOXRT.GLOBAL_NAME )
    INNER JOIN $$FINLGLVWDB.N_MERAKI_REVENUE_TRX NRA
    ON NRA.MERAKI_REV_TRX_KEY=WIOXRT.DV_TRANSACTION_KEY
    INNER JOIN $$FINLGLVWDB.N_AR_TRX_LINE NATL
  ON NRA.AR_TRX_LINE_KEY=NATL.AR_TRX_LINE_KEY
     INNER JOIN $$FINLGLVWDB.N_AR_TRX NAT
      ON(NRA.AR_TRX_KEY = NAT.AR_TRX_KEY)
   ...
   LEFT OUTER JOIN $$COMREFVWDB.N_SALES_TERRITORY_TV NST
    ...
    ON ( NPT.SK_INVENTORY_ITEM_ID_INT = WIOXRT.INVENTORY_ITEM_ID) 
    LEFT OUTER JOIN $$STGDB.WI_DRVD_NCR_CMP_US_CST_MERK_CG WDNCUC
...
Target7 Name : WI_DRVD_NCR_REV_TRX_FOR_BKG_MERK_CG1


Pre SQL : 
DELETE FROM $$STGDB.WI_DRVD_NCR_REV_TRX_FOR_BKG_MERK_CG ALL;


Post SQL : 
COLLECT STATS ON $$STGDB.WI_DRVD_NCR_REV_TRX_FOR_BKG_MERK_CG;


Source8 Name : SQ_WI_DRVD_NCR_REV_FOR_BKG_MERK_CG_P_7
...
SELECT 
 ...
 CASE WHEN NRA.ATTRIBUTION_CD='ATTRIBUTED' THEN -7777 ELSE NATL.AR_TRX_LINE_KEY  END AS AR_TRX_LINE_KEY,
 ...
 FROM 
    ...
    INNER JOIN $$COMREFVWDB.N_SOURCE_SYSTEM_CODES NSSC
     ON (NSSC.GLOBAL_NAME = WIOXRT.GLOBAL_NAME)
    INNER JOIN $$STGDB.WI_DRVD_NCR_REV_TRX_FOR_BKG_MERK_CG WDNRTFB
    ...
    LEFT OUTER JOIN $$STGDB.WI_DRVD_NCR_SLS_CHL_CD_MERK_CG WDNSCC
...
Target8 Name : WI_DRVD_NCR_REV_FOR_BKG_MERK_CG1


Pre SQL : 
DELETE FROM $$STGDB.WI_DRVD_NCR_REV_FOR_BKG_MERK_CG ALL;


Post SQL : 
COLLECT STATS ON $$STGDB.WI_DRVD_NCR_REV_FOR_BKG_MERK_CG;


Source9 Name : SQ_EX_OM_XXNCR_REV_MERK_CG_P_8
...
SELECT 
   ...
   FROM   
    $$STGDB.WI_OM_XXNCR_REV_MERK_CG WIOXRT
    WHERE
     NOT EXISTS (SELECT 1 FROM $$STGDB.WI_DRVD_NCR_REV_FOR_BKG_MERK_CG WDNRTFB
...
Target9 Name : EX_OM_XXNCR_REV_MERK_CG1


Pre SQL : 
DELETE FROM $$EXCEPDB.EX_OM_XXNCR_REV_MERK_CG ALL;


Post SQL : 
CALL COLLECT_STATS_WRAP('$$EXCEPDB','EX_OM_XXNCR_REV_MERK_CG','D');