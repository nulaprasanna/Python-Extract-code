...
AS 
(SELECT TRX.*,NSOL.ORIGINAL_SYSTEM_LINE_REF_ID 
FROM (SELECT DISTINCT SO_SBSCRPTN_ITM_SLS_TRX_KEY,SK_TRX_ID_INT,
...
INNER JOIN (SELECT BK_LINE_REF_NUM,SALES_ORDER_LINE_KEY,ORIGINAL_SYSTEM_LINE_REF_ID
...
INNER JOIN $$SLSORDVWDB.N_SALES_ORDER_LINE NSOL
ON NSOL.SALES_ORDER_LINE_KEY=POS_TV.SALES_ORDER_LINE_KEY )  WITH DATA
...
ELSE 
COALESCE(( CAST(SOL.RU_SERVICE_CONTRACT_START_DTM AS DATE)-CAST(NSOL.RU_SERVICE_CONTRACT_END_DTM AS DATE)) ,-9999) END ) AS RENEWAL_GAP_DAYS
FROM
ST_OPL_XXGCO_RTR_ATTRIB_CV_R RTR
INNER JOIN  $$SLSORDVWDB.N_SALES_ORDER_LINE SOL
ON SOL.SK_SO_LINE_ID_INT=RTR.ERP_LINE_ID
INNER  JOIN   $$SLSORDVWDB.N_SALES_ORDER_LINE NSOL 
ON (CASE WHEN RENEWAL_ERP_REF_VALUE<>'UNKNOWN' THEN NSOL.SK_SO_LINE_ID_INT
	 ELSE NSOL.ORIGINAL_SYSTEM_LINE_REF_ID END) =
(CASE WHEN RENEWAL_ERP_REF_VALUE<>'UNKNOWN' THEN  CAST(RENEWAL_ERP_REF_VALUE AS BIGINT) ELSE  RTR.RENEWAL_REF_VALUE END)  
...
ELSE
COALESCE(( CAST(SOL.RU_SERVICE_CONTRACT_START_DTM AS DATE)-CAST(OA_R.RU_SERVICE_CONTRACT_END_DTM AS DATE)) ,-9999) END) RENEWAL_GAP_DAYS
FROM
ST_OPL_XXGCO_RTR_ATTRIB_CV_R RTR
INNER JOIN  $$SLSORDVWDB.N_SALES_ORDER_LINE SOL
ON SOL.SK_SO_LINE_ID_INT=RTR.ERP_LINE_ID
INNER JOIN (SELECT SOL_R.SALES_ORDER_LINE_KEY,SK_SO_LINE_ID_INT,SK_OFFER_ATTRIBUTION_ID_INT,RU_SERVICE_CONTRACT_START_DTM,RU_SERVICE_CONTRACT_END_DTM  FROM  $$SLSORDVWDB.N_SOL_OFFER_ATTRIBUTION OA INNER JOIN $$SLSORDVWDB.N_SALES_ORDER_LINE SOL_R
 ON SOL_R.SALES_ORDER_LINE_KEY=OA.TOP_SKU_SALES_ORDER_LINE_KEY)OA_R
...
ELSE
COALESCE(( CAST(O.TERM_START_DT AS DATE)-CAST(SCL.SVC_CNTRCT_LINE_END_DTM AS DATE)) ,-9999) END) AS RENEWAL_GAP_DAYS
FROM
ST_OPL_XXGCO_RTR_ATTRIB_CV_R RTR
INNER JOIN  XAAS_R O
...
ELSE
COALESCE(( CAST(O.TERM_START_DT AS DATE)-CAST(R.TERM_END_DT AS DATE)) ,-9999) END)AS RENEWAL_GAP_DAYS
...
WHERE RENEWAL_REF_TYPE IN ('LINE REFERENCE NUMBER'))XAAS
SET DV_RENEWAL_GAP_DAYS_CNT=XAAS.RENEWAL_GAP_DAYS
...
ON  RTR.ORDER_NUMBER=O.WEB_ORDER_ID
AND RTR.TRX_ID=O.SK_TRX_ID_INT
LEFT JOIN (SELECT TRX.*,SK_ATTRIBUTION_ID_INT from XAAS_R TRX INNER JOIN 
...
WHERE RENEWAL_REF_TYPE IN ('OFFER ATTRIBUTION ID'))XAAS
SET DV_RENEWAL_GAP_DAYS_CNT=XAAS.RENEWAL_GAP_DAYS
WHERE WI.SK_RTR_ATTRIBUTION_ID=XAAS.RTR_ATTRIBUTION_ID
AND WI.SK_ATTRIBUTION_ID_INT=XAAS.OA_ATTRIBUTION_ID
AND WI.ITEM_CATEGORY_NAME IN ( 'Billing', 'FLEXIBLE CONSUMPTION MODEL' ) 
...
ELSE
COALESCE(( CAST(BKG_POS.RU_SERVICE_CONTRACT_START_DTM  AS DATE)-CAST((CASE WHEN BK_SALES_CHANNEL_CD ='Two Tier Distributor' THEN POS.RU_SERVICE_CONTRACT_END_DTM ELSE NSOL.RU_SERVICE_CONTRACT_END_DTM END)    AS DATE)) ,-9999) END ) AS RENEWAL_GAP_DAYS
FROM
...
INNER JOIN   $$SLSORDVWDB.N_SALES_ORDER_LINE NSOL
ON (CASE WHEN RENEWAL_ERP_REF_VALUE<>'UNKNOWN' THEN NSOL.SK_SO_LINE_ID_INT
	 ELSE NSOL.ORIGINAL_SYSTEM_LINE_REF_ID END)=(CASE WHEN RENEWAL_ERP_REF_VALUE<>'UNKNOWN' THEN CAST(RENEWAL_ERP_REF_VALUE AS BIGINT) ELSE RENEWAL_REF_VALUE END)
LEFT  JOIN POS_R POS 
...
ELSE
COALESCE(( CAST(BKG_POS.RU_SERVICE_CONTRACT_START_DTM  AS DATE)-CAST((CASE WHEN BK_SALES_CHANNEL_CD='Two Tier Distributor' THEN POS_REF.RU_SERVICE_CONTRACT_END_DTM ELSE NSOL.RU_SERVICE_CONTRACT_END_DTM END)    AS DATE)) ,-9999) END) AS RENEWAL_GAP_DAYS
FROM
(SELECT DISTINCT RENEWAL_ERP_REF_VALUE,RTR_ATTRIBUTION_ID,OA_ATTRIBUTION_ID,TRX_ID,ERP_SO_NUMBER,ERP_LINE_ID,ORDER_NUMBER, LINE_REF_NUMBER ,ITEM_CATEGORIZATION,RENEWAL_REF_TYPE,RENEWAL_REF_VALUE ,SALES_MOTION,POS_TRANSACTION_ID_INT
FROM $$STGDB.ST_OPL_XXGCO_RTR_ATTRIB_CV
...
INNER JOIN  $$SLSORDVWDB.N_BKG_POS_TRANSACTION_LINE BKG_POS  
ON RTR.POS_TRANSACTION_ID_INT = BKG_POS.BK_POS_TRANSACTION_ID_INT
INNER JOIN (SELECT SOL_R.SALES_ORDER_LINE_KEY,BK_SALES_CHANNEL_CD,SK_OFFER_ATTRIBUTION_ID_INT,RU_SERVICE_CONTRACT_START_DTM,RU_SERVICE_CONTRACT_END_DTM  FROM  $$SLSORDVWDB.N_SOL_OFFER_ATTRIBUTION OA INNER JOIN $$SLSORDVWDB.N_SALES_ORDER_LINE SOL_R 
ON SOL_R.SALES_ORDER_LINE_KEY=OA.TOP_SKU_SALES_ORDER_LINE_KEY)NSOL
