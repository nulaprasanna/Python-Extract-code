...
AND WBM.DV_SALES_ORDER_LINE_KEY < 0; /* AR NAPLD UPDATE CONDITION FOR AR MNL UPLD*/
 ...
 UPDATE WBM
  ...
  WHERE SALES_ORDER_LINE_KEY > 0 AND RTNR_ELGIBILITY_FLG = 'N' AND SS_CODE IN ('CG','OPL')
...
SELECT 
  ...
  NBM.BOOKINGS_PROCESS_DATE,
  NBM.BKGS_MEASURE_TRANS_TYPE_CODE ,
  CASE WHEN NATL.ORIGIN_SOURCE_CD IN ('BSFT-OPL','CBM-OPL-CLOUD','CBM-OPL-ICMS') AND  NSOL.RU_SERVICE_CONTRACT_START_DTM  IS NULL AND  
 NSOL.PERIODIC_BILLING_START_DT  IS NOT NULL THEN  NSOL.PERIODIC_BILLING_START_DT ELSE
  CAST(NSOL.RU_SERVICE_CONTRACT_START_DTM AS DATE )  END  CONTRACT_START_DTM,
   CASE WHEN NATL.ORIGIN_SOURCE_CD IN ('BSFT-OPL','CBM-OPL-CLOUD','CBM-OPL-ICMS') AND  NSOL.RU_SERVICE_CONTRACT_END_DTM  IS NULL AND  
 NSOL.PERIODIC_BILLING_END_DT   IS NOT NULL THEN  NSOL.PERIODIC_BILLING_END_DT ELSE
  CAST(NSOL.RU_SERVICE_CONTRACT_END_DTM AS DATE ) END  CONTRACT_END_DTM,
  ROUND(CASE WHEN ( CONTRACT_START_DTM - CONTRACT_END_DTM )  IS   NOT NULL 
                                                                  THEN  CAST((CAST(CONTRACT_END_DTM -  CONTRACT_START_DTM AS INTEGER)*12(FLOAT))/365 AS DECIMAL(15,6) )
   ...
   WHEN ( NP.BK_PRODUCT_ID LIKE 'SWOA%' OR  NP.BK_PRODUCT_ID LIKE '%ADJ%' )AND NP.BK_PRODUCT_ID NOT LIKE 'SWOA-MX%'  THEN NULL
     ELSE CASE
	  	  WHEN NATL.ORIGIN_SOURCE_CD in ('BSFT-OPL','CBM-OPL-CLOUD','CBM-OPL-ICMS') THEN 1
...
'16621975','16922373','6922383','16897335','16901905','17684649'))  AND NP.RU_BK_PRODUCT_FAMILY_ID='WBXEE'  THEN 'Y'
  ...
  ON NATL.AR_TRX_LINE_KEY=NBM.DV_AR_TRX_LINE_KEY
  INNER  JOIN (SELECT 
...
/*  UPDATE NBM FROM 
 ...
 -- SOL look up
 CASE
 WHEN NATL.ORIGIN_SOURCE_CD IN ('BSFT-OPL','CBM-OPL-CLOUD') AND  SOL.RU_SERVICE_CONTRACT_START_DTM  IS NULL AND  
 SOL.PERIODIC_BILLING_START_DT  IS NOT NULL THEN  SOL.PERIODIC_BILLING_START_DT
 ...
 CASE
 WHEN NATL.ORIGIN_SOURCE_CD IN ('BSFT-OPL','CBM-OPL-CLOUD') AND  SOL.RU_SERVICE_CONTRACT_END_DTM  IS NULL AND   
 SOL.PERIODIC_BILLING_END_DT   IS NOT NULL THEN  SOL.PERIODIC_BILLING_END_DT
 ...
 FROM $$WORKDB.W_BOOKINGS_MEASURE WBM
 INNER JOIN $$FINLGLVWDB.N_AR_TRX_LINE NATL
 ON NATL.AR_TRX_LINE_KEY=WBM.DV_AR_TRX_LINE_KEY
 AND NATL.SS_CODE IN ('CG','OPL')
