...
WHERE BKGS_MEASURE_TRANS_TYPE_CODE='XAAS'  AND ( FISCAL_YEAR_MONTH_INT = (SELECT FISCAL_YEAR_MTH_NUMBER_INT FROM $$ETLVWDB.BKG_PROCESS_DT_CNTRL)
OR  BOOKINGS_MEASURE_KEY IN ( SELECT  BOOKINGS_MEASURE_KEY  FROM $$SLSORDVWDB.N_BKGS_MEASURE  WHERE  BKGS_MEASURE_TRANS_TYPE_CODE='XAAS'   AND 
...
WHERE  BKG.BKGS_MEASURE_TRANS_TYPE_CODE='XAAS' AND MT.END_TV_DTM='3500-01-01 00:00:00' AND
...
BV_BKGS_MEASURE.DV_FMV_FLG,BV_BKGS_MEASURE.SK_SALES_MOTION_ATTRIB_KEY,RTR.SALES_ORDER_LINE_KEY    AS SMR_SALES_ORDER_LINE_KEY,
RTR.SALES_MOTION_CD
...
(BV_BKGS_MEASURE.DD_COMP_US_NET_PRICE_AMOUNT*Coalesce(RTR.DV_ALLOCATION_PCT,1)*Coalesce(SFH.ALLOCATION_PERCENTAGE,1)*Coalesce(BE.PRDT_FAMILY_ALLOCATION_PCT,1)) AS DD_COMP_US_NET_PRICE_AMOUNT1,
...
BV_BKGS_MEASURE.DV_SO_SBSCRPTN_ITM_SLS_TRX_KEY   AS MANUAL_TRX_KEY,BV_BKGS_MEASURE.BK_SALES_ADJ_LINE_NUMBER_INT,BV_BKGS_MEASURE.BK_SALES_ADJ_NUMBER_INT
,NULL AS IDE_IFC_COMMENT_1_TXT, NULL AS IDE_IFC_COMMENT_2_TXT, NULL AS IDE_IFC_COMMENT_3_TXT, NULL AS IDE_IFC_COMMENT_4_TXT, NULL AS IDE_IFC_COMMENT_5_TXT, NULL AS IDE_IFC_DESCRIPTION_TXT,
...
RTR.ATTR_PRDT_OFFER_TYPE_NAME||RTR.SALES_MOTION_CD||CASE WHEN RENEWAL_REF_CD IN ( 'RENEW COVERED LINE ID','SUBSCRIPTION REFERENCE ID',
'RENEW INSTANCE ID') THEN RENEWAL_REF_ID ELSE NULL END|| CASE WHEN RENEWAL_REF_CD IN ( 'RENEW COVERED LINE ID','SUBSCRIPTION REFERENCE ID',
'RENEW INSTANCE ID') 
...
SOL.WEB_ORDER_ID,             
SOL.ORDER_CREATED_DT,              
SOL.SUBSCRIPTION_REFERENCE_ID,     
EXAAS.SUBSCRIPTION_START_DT,
EXAAS.SUBSCRIPTION_END_DT,
...
INNER JOIN (SELECT * FROM $$NRTNCRVWDB.N_XAAS_SO_SBSCR_ITM_SLSTRX_NRT WHERE DV_TRX_LINE_CREATION_DT <> '3500-01-01' ) SOL
ON SOL.SO_SBSCRPTN_ITM_SLS_TRX_KEY = BV_BKGS_MEASURE.DV_SO_SBSCRPTN_ITM_SLS_TRX_KEY
LEFT JOIN $$EXAASVWDB.N_EXAAS_SUBSCRIPTION  EXAAS
ON SOL.SRC_RPTD_SBSCRPTN_CD = EXAAS.BK_EXAAS_SUBSCRIPTION_NUM 
AND SOL.DV_LINKED_MJR_LN_REF_NUM_KEY = EXAAS.LINKED_LINE_REF_KEY_INT 
AND SOL.RENEWAL_CNT= EXAAS.SUBSCRIPTION_RENEW_CNT
...
ON SO.SALES_ORDER_KEY = BV_BKGS_MEASURE.SALES_ORDER_KEY
...
WHERE BV_BKGS_MEASURE.BKGS_MEASURE_TRANS_TYPE_CODE IN ('XAAS')
...
AND RPT.BKGS_MEASURE_TRANS_TYPE_CODE= 'XAAS'
...
WHERE BKGS_MEASURE_TRANS_TYPE_CODE = 'XAAS' GROUP BY 1,2,3) BKGS
...
WHERE RPT.BKGS_MEASURE_TRANS_TYPE_CODE= 'XAAS'
GROUP BY 1,2,3,4
) OFFER
