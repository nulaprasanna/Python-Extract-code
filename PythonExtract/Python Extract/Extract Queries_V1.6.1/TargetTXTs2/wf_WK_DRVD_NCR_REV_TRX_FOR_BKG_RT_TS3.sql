...
,WDRTO.POB_TYPE_CD
   ...
   ,WDNRFB.DV_MULTIYEAR_US_NET_AMT
    ,CASE WHEN NP.ACV_FLG='Y' THEN 'ACV' ELSE 'TCV' END AS BOOKINGS_POLICY_CD1
  FROM 
...
,WDRTO.POB_TYPE_CD
   ...
   ,WDNRFB.DV_CONTRACT_DURATION
   ,WDNRFB.DV_ANNUALIZED_FLG
   ,-1 * WDNRFB.DV_ANNUALIZED_US_NET_AMT
   ,-1 * WDNRFB.DV_MULTIYEAR_US_NET_AMT
   ,CASE WHEN NP.ACV_FLG='Y' THEN 'ACV' ELSE 'TCV' END AS BOOKINGS_POLICY_CD1
  FROM 
  $$STGDB.WI_DRVD_NCR_REV_FOR_BKG_RT WDNRFB
  INNER JOIN $$STGDB.WI_DRVD_NCR_REV_TRX_BKG_RT_OA WDRTO
...
,BNDL_OFFSET.POB_TYPE_CD
   ...
   ,BNDL_OFFSET.DV_ANNUALIZED_US_NET_AMT
   ,BNDL_OFFSET.DV_MULTIYEAR_US_NET_AMT
   ,BNDL_OFFSET.BOOKINGS_POLICY_CD
   ,BNDL_OFFSET.ACTION_CODE
   ,BNDL_OFFSET.DML_TYPE  
  FROM  
...
WHERE WK.REVENUE_TRANSFER_KEY=NRT.REVENUE_TRANSFER_KEY
AND WK.DV_SALES_ORDER_LINE_KEY=NSOL.SALES_ORDER_LINE_KEY
AND WK.PRODUCT_KEY=NSOL.PRODUCT_KEY   
AND NSOL.SS_CODE NOT IN ('OPL') AND NRT.TRANSACTION_SOURCE_CD IN ('ERP')
...
$$WORKDB.W_DRVD_NCR_REV_TRX_FOR_BKG WK,
$$FINLGLVWDB.N_REVENUE_TRANSFER NRT,
$$NRTNCRVWDB.N_SOL_OFFER_ATTRIBUTION NSOL_OA
SET BOOKINGS_POLICY_CD=COALESCE(NSOL_OA.BOOKINGS_POLICY_CD,'TCV')            
WHERE WK.REVENUE_TRANSFER_KEY=NRT.REVENUE_TRANSFER_KEY 
AND WK.DV_SALES_ORDER_LINE_KEY=NSOL_OA.TOP_SKU_SALES_ORDER_LINE_KEY
AND NRT.OFFER_ATTRBTN_ID_INT=NSOL_OA.SK_OFFER_ATTRIBUTION_ID_INT   
AND WK.DV_SALES_ORDER_LINE_KEY>0 AND NRT.TRANSACTION_SOURCE_CD IN ('ERP')
AND WK.CREATED_BY_INT=1 AND WK.DV_ATTRIBUTION_CD IN ('ATTRIBUTED');


-- APPLIED XAAS TOPSKU (RTE)
...
WHERE WK.REVENUE_TRANSFER_KEY=NRT.REVENUE_TRANSFER_KEY
AND NRT.SUBSCRIPTION_ITEM_CHANGE_ID_INT=NXAAS.SBSCRPTN_ITEM_CHANGE_ID_INT
AND WK.PRODUCT_KEY=NXAAS.SUBSCRIPTION_PRODUCT_KEY   
AND NRT.TRANSACTION_SOURCE_CD='XAAS'
AND NRT.SUBSCRIPTION_ITEM_CHANGE_ID_INT>0 AND WK.CREATED_BY_INT=1;

-- APPLIED XAAS SUBSKU (RTE)

UPDATE WK FROM 
$$WORKDB.W_DRVD_NCR_REV_TRX_FOR_BKG WK,
$$FINLGLVWDB.N_REVENUE_TRANSFER NRT,
$$SLSORDVWDB.N_XAAS_SO_SBSCRPTN_ATTRBTN NXAAS
SET BOOKINGS_POLICY_CD=COALESCE(NXAAS.BOOKINGS_POLICY_CD,'TCV')            
WHERE WK.REVENUE_TRANSFER_KEY=NRT.REVENUE_TRANSFER_KEY
AND NRT.OFFER_ATTRBTN_ID_INT=NXAAS.BK_OFFER_ATTRIBUTION_ID_INT   
--AND WK.PRODUCT_KEY=NXAAS.SUBSCRIPTION_PRODUCT_KEY   
AND NRT.TRANSACTION_SOURCE_CD='XAAS' AND WK.DV_ATTRIBUTION_CD IN ('ATTRIBUTED')
AND NRT.SUBSCRIPTION_ITEM_CHANGE_ID_INT>0 AND WK.CREATED_BY_INT=1;
 ...
 ( 
	 ...
	 (
		 ...
		 SEL BK_TRANSACTION_SEQ_ID_INT, BK_SS_CD FROM $$STGDB.WI_BKG_TX_SQ_ID_FORWARD
		  UNION
         SELECT TRANSACTION_SEQUENCE_ID_INT,'CG' AS SS_CD FROM $$STGDB.WI_CONTRACT_CHANGE_RT_FWD /*Annuity net change*/
	 ) WI  
