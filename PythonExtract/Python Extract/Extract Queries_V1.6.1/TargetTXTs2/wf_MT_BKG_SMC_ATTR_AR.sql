...
Source1 Name : SQ_MT_AR_SLS_MOTION_ATTRIB
...
Target1 Name : WI_SLS_MOTION_ATTRIB_AR


Pre SQL : 
DELETE FROM $$STGDB.WI_SLS_MOTION_ATTRIB_AR;


Post SQL : 
CALL COLLECT_STATS_WRAP('$$STGDB','WI_SLS_MOTION_ATTRIB_AR','D');


Source2 Name : SQ_WI_SLS_MOTION_ATTRIB_AR
...
(
SELECT DISTINCT
SALES_ORDER_LINE_KEY AS DV_TRANSACTION_ID
,'AR' AS DV_TRX_TYPE
,DV_ENTERPRISE_INV_SKU_ID
,-9999 AS SK_OFFER_ATTRIBUTION_ID_INT
FROM $$STGDB.WI_SLS_MOTION_ATTRIB_AR
GROUP BY 1,2,3,4
)SRC
...
DELETE FROM $$STGDB.WI_MT_BKG_SPLIT_CHECK WHERE DV_TRX_TYPE = 'AR' ;
...
FROM $$STGDB.WI_SLS_MOTION_ATTRIB_AR WI
...
WHERE EXISTS ( SEL 1 FROM $$STGDB.WI_SLS_MOTION_ATTRIB_AR WI
...
FROM
$$STGDB.WI_SLS_MOTION_ATTRIB_AR WI
INNER JOIN $$ETLVWDB.SM_BKG_SLS_MOTION_ATTRIBUTION SM
  ON WI.SALES_ORDER_LINE_KEY = SM.DV_TRANSACTION_ID
  AND WI.DV_ENTERPRISE_INV_SKU_ID = SM.DV_ENTERPRISE_INV_SKU_ID
    AND WI.DV_TRX_TYPE= SM.DV_TRX_TYPE
...
AND EXISTS ( SEL 1 FROM $$STGDB.WI_MT_BKG_SPLIT_CHECK CHK
				WHERE SM.SK_SALES_MOTION_ATTRIB_KEY = CHK.SK_SALES_MOTION_ATTRIB_KEY
				  AND CHK.DV_TRX_TYPE = 'AR' ) ;

COLLECT STATS ON $$EXCEPDB.EX_MT_BKG_SLS_MOTION_ATTRIB ;
				
DELETE FROM $$STGDB.WI_SLS_MOTION_ATTRIB_AR WI
...
Source3 Name : SQ_WI_SLS_MOTION_ATTRIB_AR1
