...
REGEXP_REPLACE(REGEXP_REPLACE(TRIM(ICMS_DEAL_ID_INFO_TXT),'[^0-9 /&,A-Za-z]','',1,0,'i'),'[&/]',',',1,0,'i') ICMS_DEAL_ID_INFO_TXT,
EDW_UPDATE_DATETIME
FROM $$FINLGLVWDB.N_AR_TRX
...
ICMS_DEAL_ID_INFO_TXT IS NOT NULL 
AND EDW_UPDATE_DATETIME > '$$LastExtractDate'
...
FROM
$$STGDB.WI_AR_TRX_DEAL AR
INNER JOIN $$FINLGLVWDB.N_AR_TRX_LINE NATL
ON AR.AR_TRX_KEY = NATL.AR_TRX_KEY
...
FROM $$STGDB.WI_AR_NAPLD_SMC_DENORM WI
  WHERE NOT EXISTS ( SELECT 1 FROM $$ETLVWDB.EL_AR_NAPLD_SMC_DENORM EL
                                  WHERE WI.AR_TRX_LINE_KEY = EL.AR_TRX_LINE_KEY
                                    AND WI.DV_ENTERPRISE_INV_SKU_ID = EL.DV_ENTERPRISE_INV_SKU_ID
...
FROM $$ETLVWDB.EL_AR_NAPLD_SMC_DENORM EL,
 ( SELECT AR_TRX_LINE_KEY, DV_ENTERPRISE_INV_SKU_ID, MAX(START_TV_DTM) MAX_START_DTM
	FROM $$STGDB.WI_AR_NAPLD_SMC_DENORM WI
   GROUP BY 1,2
  ) WI
SET 
...
WHERE EL.AR_TRX_LINE_KEY = WI.AR_TRX_LINE_KEY
  AND EL.DV_ENTERPRISE_INV_SKU_ID = WI.DV_ENTERPRISE_INV_SKU_ID
