...
--       QUALIFY ROW_NUMBER() OVER(PARTITION BY BK_POS_TRANSACTION_ID_INT ORDER BY SALES_ORDER_LINE_KEY DESC) = 1  ) ) 
	   */
	   (SELECT * FROM $$SLSORDVWDB.MT_POS_RTNR_SMC_ALLOCATION MTPOS
	WHERE MTPOS.END_TV_DTM='3500-01-01 00:00:00'    AND MTPOS.EDW_UPDATE_DTM > '$$LastExtractDate' )POS  
   ...
   AND NSLSADJ.END_TV_DTM = '3500-01-01 00:00:00'
     AND NSLSADJ.RU_BK_NEW_RENEW_POS_TRX_ID_INT > 0
	   INNER JOIN (SELECT BK_POS_TRANSACTION_ID_INT,SALES_ORDER_LINE_KEY FROM $$SLSORDVWDB.N_POS_TRX_LN_AS_NEW_OR_RNWL_TV 
WHERE END_TV_DTM = '3500-01-01 00:00:00'
GROUP BY 1,2)  PNVR
ON  PNVR.BK_POS_TRANSACTION_ID_INT= POS.BK_POS_TRANSACTION_ID_INT
...
AND SALES_MOTION_CD='RENEW'
	QUALIFY ROW_NUMBER() OVER(PARTITION BY SALES_ORDER_LINE_KEY ORDER BY EDW_UPDATE_DTM DESC ) = 1)SMC_ALLOC/* To	avoid data explosion in MT_RTNR_SMC_ALLOCATION*/
       ON PNVR.SALES_ORDER_LINE_KEY = SMC_ALLOC.SALES_ORDER_LINE_KEY      
 LEFT JOIN $$STGDB.WI_TTS_SOWB_SOL_LINES NSOL
           ON PNVR.SALES_ORDER_LINE_KEY = NSOL.SALES_ORDER_LINE_KEY
